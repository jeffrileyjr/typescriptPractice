// declare an interface called Camera
interface Camera {
    brand: string;
    model: string;
    price: number;
...
function findCameras(arrayofCameras: Camera[], model: string): Camera[] | void {
...
function addCamera(arrayofCameras: Camera[], brand: string, model:string, price: number, fixedLens: boolean): void {
    // we just need to make a variable to hold our new object, then push it into our camera array
    let newCamera: Camera = {brand, model, price, fixedLens};
...
function areThereAnyFixedLensForSale(arrayofCameras: Camera []): boolean {
    const fixedLensCameras: Camera [] = arrayofCameras.filter(camera => camera.fixedLens);
...
function collectionValue(arrayofCameras: Camera[]): number {
    let value: number = 0;
